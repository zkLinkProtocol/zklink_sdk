...
use zklink_sdk_types::basic_types::pack::{closest_packable_fee_amount, closest_packable_token_amount};
...
pub fn closest_packable_transaction_amount(amount: &str) -> Result<String, JsValue> {
    let amount = BigUint::from_str(amount).map_err(|e| TypeError::InvalidBigIntStr(e.to_string()))?;
